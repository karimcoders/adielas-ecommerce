  ...
  return (
    ...
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
        ...
        {/* clip cards — sticky stacking deck while you scroll */}
        <div className="mt-12 pb-10">
          {clips.map((clip, i) => (
            <div
              key={clip.title}
              className="sticky-card mx-auto w-full max-w-xl px-0 sm:max-w-2xl"
              style={{ top: `calc(5.5rem + ${i * 2.25}rem)`, zIndex: i + 1 }}
            >
              <Reveal variant="zoom" delay={i * 90}>
                <button
                  type="button"
                  className="group relative flex h-[21rem] w-full items-center justify-center overflow-hidden rounded-[2rem] bg-white/40 shadow-[0_18px_44px_rgba(20,56,15,0.20)] ring-1 ring-white/50 backdrop-blur-sm transition-transform duration-300 hover:-translate-y-1.5 sm:h-[24rem]"
                ...
                >
                  ...
                  <span className="font-display absolute bottom-4 left-5 text-left text-xl text-white drop-shadow sm:text-2xl">
                    {clip.title}
                  </span>
                </button>
              </Reveal>

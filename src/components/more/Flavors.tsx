...
import { defaultFlavors } from "@/lib/cms-defaults";

type FlavorsCms = Partial<typeof defaultFlavors>;
...
export function Flavors({ cms }: { cms?: FlavorsCms }) {
  const c = { ...defaultFlavors, ...(cms ?? {}) };
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
        <Reveal>
          <h2 className="mx-auto max-w-4xl text-center text-4xl font-semibold leading-tight tracking-tight text-[var(--forest)] sm:text-5xl lg:text-[3.4rem]">
            {c.headingLine1}
            <br />
            <span className="text-[var(--sage-deep)]">{c.headingLine2}</span>
          </h2>
        </Reveal>
      </div>

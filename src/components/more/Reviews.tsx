...
import { defaultReviews } from "@/lib/cms-defaults";

type ReviewsCms = Partial<typeof defaultReviews>;
...
export function Reviews({ cms }: { cms?: ReviewsCms }) {
  const c = { ...defaultReviews, ...(cms ?? {}) };
  const reviews = Array.isArray(c.items) && c.items.length > 0 ? c.items : defaultReviews.items;

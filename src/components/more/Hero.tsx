...
type HeroCms = Partial<typeof defaultHero>;

function BuyButton({ className = "", text = "Shop now", link = "/shop" }: { className?: string; text?: string; link?: string }) {
  return (
    <Link
      href={link}
...
function StatBubbles({
  ...
  stats: { to: number; unit: string; label: string }[];
}) {
  return (
    <>
      {stats.slice(0, 3).map((s, i) => (
        <Reveal
          ...
          variant="zoom"
          delay={DELAYS[i] ?? 300}
          className={`absolute z-10 ${BUBBLE_POS[i]}`}
        >
          <div
...
export function Hero({ cms }: { cms?: HeroCms }) {
  const c = { ...defaultHero, ...(cms ?? {}) };
  const stats = (c.stats && Array.isArray(c.stats) && c.stats.length === 3
    ? c.stats
    : defaultHero.stats) as { to: number; unit: string; label: string }[];
        ...
        <div className="order-1 pt-2 text-center lg:order-2 lg:pt-0">
          ...
          <Reveal delay={200}>
            ...
            <Reveal delay={260} variant="down" className="mt-5">
              <BuyButton text={c.ctaText} link={c.ctaLink} />
            </Reveal>
          </Reveal>
        </div>
      </div>
